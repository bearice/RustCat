...
pub struct WindowsSettingsManager;

impl SettingsManager for WindowsSettingsManager {
    ...
    fn migrate_legacy_settings() {
        ...
        if let Ok(sub_key) = key.open_subkey_with_flags("Software\\RustCat", KEY_READ) {
            ...
            if let Ok(old_id) = sub_key.get_value::<u32, &str>("IconId") {
                ...
                // Save new settings
                Self::set_current_icon(&icon_name);
                Self::set_current_theme(Some(theme));
                ...
                println!(
                    "Migrated from legacy IconId {} to IconName: {}, Theme: {}",
                    old_id, icon_name, theme
                );
            }
        }
    }
